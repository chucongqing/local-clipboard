...
- **MessageStore** — in-memory message history protected by `sync.RWMutex`. Stores text and file metadata so newly connected clients can sync the current session; cleared alongside FileStore.
...
**File sharing flow:** Client uploads file via `POST /upload` → backend streams the file to disk, stores metadata in FileStore, and returns a file ID → frontend sends a WebSocket message with the file ID (no content) → Hub persists the metadata in MessageStore and broadcasts to all clients → each client fetches `/file/:id` to download.

**Auto-clear timer:** Hub owns a `ClearConfig` (interval, paused, nextClearTime) and communicates with three buffered channels (`clearNowCh`, `setIntervalCh`, `togglePauseCh`). A nil `<-chan time.Time` (the nil-channel trick) disables the timer without a separate flag. On any clear, Hub clears both FileStore and MessageStore, then broadcasts `type:"clear"`; on config changes it broadcasts `type:"config"`. New clients receive the current config and a `type:"history"` message with all persisted messages immediately on connect. The default interval is **10 minutes** — the timer is started immediately in `hub.run()` when `IntervalMin > 0`.
